var sentiment = function (data) {
  data = _.keys(data).map(function(key) {return {source: key, sentiment: data[key]};}).filter(function (x) {
    return !_.isNull(x.sentiment);
  });
  ...
  var tip = d3.tip()
    ...
    .offset([-10, 0])
    .html(function(d) {
      return "<strong>Sentiment Value:</strong> <span style='color:red'>" + d.sentiment + "</span>";
    })
